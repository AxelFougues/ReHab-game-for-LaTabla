...
vec2 RehabWorld::resolveCollisionWithContours ( vec2 point, float radius, const Shape* s ){
...
vec2 RehabWorld::resolveCollisionWithInverseContours( vec2 point, float radius, const Shape* s ){
    const Contour* in = mContours.findLeafContourContainingPoint(point,mContourFilter) ;
    // ok, find closest
    if (in){
        if ( !in->mIsHole ){
...
vec2 RehabWorld::getDenoisedShapeVel( const Shape& s) const{
    vec2 vel;
...
vec2 RehabWorld::unlapEdge( vec2 p, float r, const Contour& poly, const Shape* s ){
    float dist ;
...
vec2 RehabWorld::unlapHoles( vec2 p, float r, ContourKind kind, const Shape* s ){
    float dist ;
    vec2 x ;
...
void RehabWorld::onShapeShapeCollide( const Shape& a, const Shape& b ){
    ShapeShapeCollisions.push_back( ShapeShapeCollision(getShapeIndex(&a),getShapeIndex(&b)));
}

void RehabWorld::onShapeContourCollide( const Shape& a, const Contour& b, vec2 pt ){
    ShapeContourCollisions.push_back(ShapeContourCollision( getShapeIndex(&a), mContours.getIndex(b), pt ));
}

void RehabWorld::onShapeWorldBoundaryCollide  ( const Shape& a, vec2 pt ){
    ShapeWorldCollisions.push_back( ShapeWorldCollision(getShapeIndex(&a),pt) );
}

void RehabWorld::scaleShapeVelsForIntegrationSteps( int oldSteps, int newSteps ){
...
void RehabWorld::resolveShapeContourCollisions(){
    for( auto &b : shapes ){
...
void RehabWorld::resolveShapeCollisions(){
    if ( shapes.size()==0 )
        return ;
